...
def homePage():
	Button(root, text="Play", command=gamePage, font="Arial 20 bold", bg="white", fg="#9966FF", width=20).place(relx = 0.5, rely=0.3, anchor="center")
	Button(root, text="Leader board", command=openLeaderboard,font="Arial 20 bold", bg="white", fg="#9966FF", width=20).place(relx = 0.5, rely=0.4, anchor="center")
	Button(root, text="About the game", command=aboutPage, font="Arial 20 bold", bg="white", fg="#9966FF", width=20).place(relx = 0.5, rely=0.5, anchor="center")
	Button(root, text="Exit", command=root.quit, font="Arial 20 bold", bg="white", fg="#9966FF", width=20).place(relx = 0.5, rely=0.6, anchor="center")
...
def gamePage():
	...
	Button(canvas,text="Save Game").place(rely=0,width=1024,height=buttonHeight)
...
def moveSnake():
	...
	for i in range(1,len(snake)):
		...
		if sHeadPos[0] < list1[2] and sHeadPos[2] > list1[0] and sHeadPos[1] < list1[3] and sHeadPos[3] > list1[1]:
			...
			canvas.create_image(width/2,height/2 - 100, image=gameOverPic, anchor="c")
			root.update()
	...
	if len(snake) > snakeSize:
		snakeActive = snakeSize
	else:
		snakeActive = len(snake)
	for i in range(1,snakeActive):
		positions.append(canvas.coords(snake[i]))

	for i in range(snakeActive-1):
		canvas.coords(snake[i+1],positions[i][0],positions[i][1],positions[i][2],positions[i][3])



def growSnake():
	global SCORE, snake, snakeSize, direction
